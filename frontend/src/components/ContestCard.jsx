...
import { FaBookmark, FaExternalLinkAlt, FaRegBookmark, FaYoutube } from 'react-icons/fa';
...
const ContestCard = ({ contest }) => {
  ...
  const contestUrl = `https://${urlPrefix[contest.platform.toLowerCase()]}/${contest.code}`;
  ...
  const toggleBookmark = () => {
    ...
    if (isBookmarked) {
      const updatedBookmarks = bookmarks.filter((elem) => elem.code != contest.code);
  ...
  return (
    <div className={`contest-card ${darkMode ? 'dark' : 'light'}`}>
      ...
      <div className='card-actions'>
        <a href={contestUrl} target='_blank' rel='noopener noreferrer' className='contest-link'>
